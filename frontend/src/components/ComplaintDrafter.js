...
import { generateAIDocument } from '../services/api';
...
const ComplaintDrafter = ({ template, onBack }) => {
  ...
  const generateComplaint = async () => {
    ...
    try {
      ...
      const aiResult = await generateAIDocument(complaintData);
      
      if (aiResult.success) {
        setGeneratedComplaint(aiResult.document);
      } else {
        // Fallback to local generation if AI fails
        console.warn('AI generation failed, using fallback:', aiResult.error);
        const fallbackComplaint = generateComplaintText(formData, template);
        setGeneratedComplaint(fallbackComplaint + '\n\n[Note: Generated using fallback method as AI service was unavailable]');
      }
      
      setShowPreview(true);
    } catch (error) {
      console.error('Error in complaint generation:', error);
      // Use fallback method
      const fallbackComplaint = generateComplaintText(formData, template);
            ...
            >
              {isGenerating ? (
                <>
                  ...
                  AI is Generating Legal Document...
                </>
              ) : (
                <>
